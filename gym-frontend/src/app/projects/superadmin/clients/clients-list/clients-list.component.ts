  ...
  columns: ColumnDef[] =
    [
      ...
      },
      {
        ...
        actions: ['edit', 'copy', 'delete', 'activate', 'deactivate'],
        actionCondition: (row) => {
          const base: TableActionType[] = ['edit', 'copy', 'delete'];
          if (['suspended','inactive'].includes(row.status)) return [...base, 'activate'];
          if (['active','trial'].includes(row.status) ) return [...base, 'deactivate'];
          return base;
        }
      }
    ];
